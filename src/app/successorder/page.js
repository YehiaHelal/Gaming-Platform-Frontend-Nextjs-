...
const BlogPage = () => {
  ...
  useEffect(() => {
    ...
    const token = JSON.parse(localStorage.getItem("Gaminguser"));
    ...
    const finilizestripepay = async (OrderDetails) => {
      ...
      try {
        ...
        if ((datas.data.status = 200)) {
          ...
          setOrderNumber(datas.data.message.ordernumber);

          // console.log(p[0].ordernumber);

          handleSendEmailToUserConfirmOrder(datas.data.message.ordernumber);
  ...
  const handleSendEmailToUserConfirmOrder = async (ordernumber) => {
    ...
    try {
      const datas = await axios.post(
        "https://www.gamingbk.shoponlinemarket.cloud/api/mail/emailorderplaced",
        {
          submission,
        },
        {
          ...
          {showError && !stripeCheckoutState && !stripeCheckoutSuccess && (
            <div>
              ...
              <h3 className={styles.sectionfourTitleh3}>
                ...
                <Link href="/services">Redirect to Cart</Link>
              </h3>
            </div>
          )}
